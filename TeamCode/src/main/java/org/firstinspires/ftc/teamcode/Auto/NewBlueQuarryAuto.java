...
@Autonomous(name="DoNotUseBlueQuarryAuto", group ="Concept")
public class NewBlueQuarryAuto extends LinearOpMode {
...
//            moveDistance(0.4, 700);
        ...
        if (SkystonePos == 1){
            gyroDrive(-90, -1.0, 74, 5.0);
        }
        else if (SkystonePos == 2){
            gyroDrive(-90, -1.0, 84, 5.0);
        }
        else{
            gyroDrive(-90, -1.0, 91, 5.0);
        }

        gyroTurn(-179, 1.0, 30, 4.0);

        gyroDrive(-180, -0.4, 15, 3.0);
        ...
        bot.arm.setTargetPosition(1800);
        bot.arm.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        bot.arm.setPower(0.6);

        while (bot.arm.isBusy()) {

            // Display it for the driver.
            telemetry.addLine("The current position: " + bot.arm.getCurrentPosition());
            telemetry.update();
        }

        bot.claw.setPosition(0.4);

        sleep(250);

        bot.arm.setPower(1);
        bot.arm.setTargetPosition(0);
        ...
        if (SkystonePos == 1){
            ...
            gyroDrive(-90, 1.0, 85, 5.0);
        }
        else{
            gyroDrive(-90, 1.0, 87, 5.0);
        }
        ...
        if(SkystonePos == 3){
            gyroDrive (-20, 0.5, 26.50, 5.0);
        }
        else {
            gyroDrive (0, 0.5, 25.0, 5.0);
        }
